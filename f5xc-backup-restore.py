...
##############################################################################################################################
def get_and_save_conf_object(backup_path, ns, base_api, items_conf_object, file_format, object_type, sleep_time):
...
##########################################
def backup_http_lb (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_http_lb, items_http_lb, 'http_lb', 'HTTP Loadbalancer', wait_time)
################################################

################################################
# Function to backup Origin Pool by namespace
################################################
def backup_origin_pool (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_origin_pool, items_origin_pool, 'origin_pool', 'Origin Pool', wait_time)
################################################

###############################################
# Function to backup Health Check by namespace
###############################################
def backup_healthchecks (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_healthchecks, items_healthchecks, 'healthcheck', 'Health Check', wait_time)
################################################

################################################
# Function to backup App firewalls by namespace
################################################
def backup_app_fw (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_app_fw, items_app_fw, 'app_fw', 'App Firewall', wait_time)      
################################################

################################################
# Function to backup Service Policy by namespace
################################################
def backup_service_policy (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_service_policy, items_service_policy, 'service_policy', 'Service Policy', wait_time)      
################################################

#####################################################
# Function to backup TCP Load Balancer by namespace
#####################################################
def backup_tcp_lb (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_tcp_lb, items_tcp_lb, 'tcp_lb', 'TCP Load Balancer', wait_time)      
################################################

######################################################
# Function to backup Forward Proxy Policy by namespace
######################################################
def backup_fwdproxy_policy (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_fwdproxy_policy, items_fwdproxy_policy, 'fwdproxy_policy', 'Forward Proxy', wait_time)      
################################################

######################################################
# Function to backup Rate Limit Policy by namespace
######################################################
def backup_ratelimiter_policy (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_ratelimiter_policy, items_ratelimiter_policy, 'ratelimiter_policy', 'Rate Limit', wait_time)      
################################################

###########################################################
# Function to backup Malicious User Mitigation by namespace
###########################################################
def backup_malicioususer_policy (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_malicioususer_policy, items_malicioususer_policy, 'malicioususer_policy', 'Malicious User Mitigation', wait_time)      
################################################

############################################################
# Function to backup User Identification Policy by namespace
############################################################
def backup_useridentification_policy (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_useridentification_policy, items_useridentification_policy, 'useridentification_policy', 'User Identification', wait_time)      
################################################

################################################
# Function to backup IP Prefix set by namespace
################################################
def backup_ip_prefixset (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_ip_prefixset, items_ip_prefixset, 'ip_prefixset', 'IP Prefix', wait_time)
################################################

################################################
# Function to backup Alert Policy by namespace
################################################
def backup_alert_policy (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_alert_policy, items_alert_policy, 'alert_policy', 'Alert Policy', wait_time)
################################################

################################################
# Function to backup Alert Receiver by namespace
################################################
def backup_alert_receiver (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_alert_receiver, items_alert_receiver, 'alert_receiver', 'Alert Receiver', wait_time)
################################################

#####################################################
# Function to backup Global Log Receiver by namespace
#####################################################
def backup_global_log_receiver (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_global_log_receiver, items_global_log_receiver, 'global_log_receiver', 'Global Log Receiver', wait_time)
################################################

#######################################################
# Function to backup Report Configuration by namespace
#######################################################
def backup_report_conf (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_report_conf, items_report_conf, 'report_conf', 'Report Configuration', wait_time)
################################################

################################################
# Function to backup API Definition by namespace
################################################
def backup_xc_api_definition (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_xc_api_definition, items_xc_api_definition, 'xc_api_definition', 'API Definition', wait_time)
################################################

################################################
# Function to backup TLS Certificate by namespace
################################################
def backup_cert_mgmt (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_cert_mgmt, items_cert_mgmt, 'cert_mgmt', 'TLS Certificate', wait_time)
################################################
   
########################################################
# Function to backup TLS Certificate Chain by namespace
########################################################
def backup_cert_mgmt_chain (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_cert_mgmt_chain, items_cert_mgmt_chain, 'cert_mgmt_chain', 'TLS Certificate Chain', wait_time)
################################################
        
#####################################################
# Function to backup Service Discovery by namespace
#####################################################
def backup_svc_discovery (backup_path,ns,wait_time):
        ...
        get_and_save_conf_object(backup_path, ns, api_svc_discovery, items_svc_discovery, 'svc_discovery', 'Service Discovery', wait_time)
################################################



##############################################################################################################################
def post_and_write_conf_object(restore_path,ns, api_url, filename_prefix, object_type, sleep_time):
...
################################################
def restore_healthchecks (restore_path,ns,wait_time):
        api_healthchecks = tenant_url + '/api/config/namespaces/' + ns + '/healthchecks'
        filename_prefix = ns + '_healthcheck-'
        post_and_write_conf_object(restore_path, ns, api_healthchecks, filename_prefix, 'Health Check', wait_time)    
################################################

################################################
# Function to Restore HTTP Loadbalancer by namespace
################################################
def restore_http_lb (restore_path,ns,wait_time):
        api_http_lb = tenant_url + '/api/config/namespaces/' + ns + '/http_loadbalancers'
        filename_prefix = ns + '_http_lb-'
        post_and_write_conf_object(restore_path, ns, api_http_lb, filename_prefix, 'HTTP LoadBalancer', wait_time)
################################################

################################################
# Function to Restore origin pool by namespace
################################################
def restore_origin_pool (restore_path,ns,wait_time):
        api_origin_pool = tenant_url + '/api/config/namespaces/' + ns + '/origin_pools'
        filename_prefix = ns + '_origin_pool-'
        post_and_write_conf_object(restore_path, ns, api_origin_pool, filename_prefix, 'Origin Pool', wait_time)
################################################

################################################
# Function to Restore TCP LB by namespace
################################################
def restore_tcp_lb (restore_path,ns,wait_time):
        api_tcp_lb = tenant_url + '/api/config/namespaces/' + ns + '/tcp_loadbalancers'
        filename_prefix = ns + '_tcp_lb-'
        post_and_write_conf_object(restore_path, ns, api_tcp_lb, filename_prefix, 'TCP LoadBalancer', wait_time)
################################################

################################################
# Function to Restore App Firewall by namespace
################################################
def restore_app_fw (restore_path,ns,wait_time):
        api_app_fw = tenant_url + '/api/config/namespaces/' + ns + '/app_firewalls'
        filename_prefix = ns + '_app_fw-'
        post_and_write_conf_object(restore_path, ns, api_app_fw, filename_prefix, 'App Firewall', wait_time)
################################################

##################################################
# Function to Restore Service Policy by namespace
##################################################
def restore_service_policy (restore_path,ns,wait_time):
        api_service_policy = tenant_url + '/api/config/namespaces/' + ns + '/service_policys'
        filename_prefix = ns + '_service_policy-'
        post_and_write_conf_object(restore_path, ns, api_service_policy, filename_prefix, 'Service Policy', wait_time)
################################################

########################################################
# Function to Restore Forword Proxy Policy by namespace
########################################################
def restore_fwdproxy_policy (restore_path,ns,wait_time):
        api_fwdproxy_policy = tenant_url + '/api/config/namespaces/' + ns + '/forward_proxy_policys'
        filename_prefix = ns + '_fwdproxy_policy-'
        post_and_write_conf_object(restore_path, ns, api_fwdproxy_policy, filename_prefix, 'Forward Proxy Policy', wait_time)
################################################

########################################################
# Function to Restore Rate Limiter Policy by namespace
########################################################
def restore_ratelimiter_policy (restore_path,ns,wait_time):
        api_ratelimiter_policy = tenant_url + '/api/config/namespaces/' + ns + '/rate_limiter_policys'
        filename_prefix = ns + '_ratelimiter_policy-'
        post_and_write_conf_object(restore_path, ns, api_ratelimiter_policy, filename_prefix, 'Rate Limiter Policy', wait_time)
################################################

########################################################
# Function to Restore Malicious User Policy by namespace
########################################################
def restore_malicioususer_policy (restore_path,ns,wait_time):
        api_malicioususer_policy = tenant_url + '/api/config/namespaces/' + ns + '/malicious_user_mitigations'
        filename_prefix = ns + '_malicioususer_policy-'
        post_and_write_conf_object(restore_path, ns, api_malicioususer_policy, filename_prefix, 'Malicious User Policy', wait_time)
################################################

#################################################
# Function to Restore IP Prefix Set by namespace
#################################################
def restore_ip_prefixset (restore_path,ns,wait_time):
        api_ip_prefixset = tenant_url + '/api/config/namespaces/' + ns + '/ip_prefix_sets'
        filename_prefix = ns + '_ip_prefixset-'
        post_and_write_conf_object(restore_path, ns, api_ip_prefixset, filename_prefix, 'IP Prefix', wait_time)
################################################

#####################################################
# Function to Restore API Definition Set by namespace
#####################################################
def restore_xc_api_definition (restore_path,ns,wait_time):
        api_xc_api_definition = tenant_url + '/api/config/namespaces/' + ns + '/api_definitions'
        filename_prefix = ns + '_xc_api_definition-'
        post_and_write_conf_object(restore_path, ns, api_xc_api_definition, filename_prefix, 'API Definition', wait_time)
################################################

#############################################################
# Function to Restore User Identification Policy by namespace
#############################################################
def restore_useridentification_policy (restore_path,ns,wait_time):
        api_useridentification_policy = tenant_url + '/api/config/namespaces/' + ns + '/user_identifications'
        filename_prefix = ns + '_useridentification_policy-'
        post_and_write_conf_object(restore_path, ns, api_useridentification_policy, filename_prefix, 'User Identification Policy', wait_time)
################################################          
...
version = '1.3' # Updated to version 1.3 to use custom backup and restore locations
...
try:
    ...
    if args['action'] == 'backup':
        ...
        for item_ns in items_ns:
            ns = item_ns
            backup_http_lb(backup_path,ns,backup_wait_time)
            backup_origin_pool(backup_path,ns,backup_wait_time)
            backup_healthchecks(backup_path,ns,backup_wait_time)
            backup_tcp_lb(backup_path,ns,backup_wait_time)
            backup_app_fw(backup_path,ns,backup_wait_time)
            backup_xc_api_definition(backup_path,ns,backup_wait_time)
            backup_service_policy(backup_path,ns,backup_wait_time)
            backup_ratelimiter_policy(backup_path,ns,backup_wait_time)
            backup_malicioususer_policy(backup_path,ns,backup_wait_time)
            backup_useridentification_policy(backup_path,ns,backup_wait_time)
            backup_ip_prefixset(backup_path,ns,backup_wait_time)
            
            #backup_fwdproxy_policy(backup_path,ns,backup_wait_time)
            #backup_alert_policy(backup_path,ns,backup_wait_time)
            #backup_alert_receiver(backup_path,ns,backup_wait_time)
            #backup_global_log_receiver(backup_path,ns,backup_wait_time)
            #backup_report_conf(backup_path,ns,backup_wait_time)
            #backup_cert_mgmt(backup_path,ns,backup_wait_time)
            #backup_cert_mgmt_chain(backup_path,ns,backup_wait_time)
            #backup_svc_discovery(backup_path,ns,backup_wait_time)
        ...
        for item_ns in items_ns:
            ns = item_ns
            restore_healthchecks(restore_path,ns,restore_wait_time)
            restore_origin_pool(restore_path,ns,restore_wait_time)
            restore_app_fw(restore_path,ns,restore_wait_time)
            restore_service_policy(restore_path,ns,restore_wait_time)
            restore_ratelimiter_policy(restore_path,ns,restore_wait_time)
            restore_ip_prefixset(restore_path,ns,restore_wait_time)
            restore_http_lb(restore_path,ns,restore_wait_time) # Note: New Auto-Cert will generate new cert.
            restore_tcp_lb(restore_path,ns,restore_wait_time) # Note: Hostname (start with ves-io-xxxx will be generate new)
            restore_malicioususer_policy(restore_path,ns,restore_wait_time)
            restore_useridentification_policy(restore_path,ns,restore_wait_time)
            
            #restore_fwdproxy_policy(restore_path,ns,restore_wait_time)
            #restore_xc_api_definition(restore_path,ns,restore_wait_time) # Need swagger file uploaded and update swagger link.
